...
import { createPost } from '$lib/modules/activitypub/apRssUtil'
...
export async function entries() {
  ...
  return blogs.chain().simplesort('date', true).data({ removeMeta: true }).map(({ id }) => ({ post_id: id }));
}
...
export async function GET({ params }) {
  try {
    ...
    const post = createPost(postMetadata);
